...
module.exports = {
    ...
    const states = Object.keys(externalStates).map(key => {
      if (key === 'thermostatMode') {
        return {
          ...
          data: {supportedThermostatModes: thermostat.supportedThermostatModes()}
        }
        ...
        return {
          ...
          attribute: this.stAttributeFor(key),
          value: this.stValueFor(key, externalStates[key]),
          data: {supportedThermostatFanModes: thermostat.supportedThermostatFanModes()}
        }
  ...
  statesForDeviceType(deviceType) {
    switch (deviceType) {
      ...
      case 'c2c-motion-5':
        return {motion: 'inactive', battery: 100, temperature: 76, illuminance: 800};
      case 'd51dc1d9-d309-4200-ae45-452c91d41d7c':
        return {motion: 'inactive', temperature: 78};
      case 'c2c-contact-3':
