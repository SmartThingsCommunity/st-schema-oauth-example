...
const db = {
	...
	addToken(username, expiresIn) {
		...
		const params = {
			...
			Item: {
				...
				expires_in: {N: expiresIn.toString()},
				token_type: {S: 'Bearer'}
			}
		};
	...
	getToken(accessToken) {
			const params = {
				...
				ProjectionExpression: "username, access_token, refresh_token, expires_in, expires, token_type, callbackAuth, callbackUrls"
			}
	...
	getCallbacks(username) {
		const params = {
			...
			ProjectionExpression: "access_token, callbackAuth, callbackUrls"
		}
	...
	addDevice(username, handlerType, displayName, states) {
	...
	createTableIfNecessary(name) {
		return dynamoDB.describeTable({"TableName": name}).promise().then(table => {
			console.log(`DB Table Name:  ${name} (exists)`)
		})
		.catch(err => {
			if (err.code === 'ResourceNotFoundException') {
				console.log(`DB Table Name:  ${name} (creating)`)
